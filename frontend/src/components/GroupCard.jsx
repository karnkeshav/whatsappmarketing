...
export default function GroupCard({ group, onChange }) {
  ...
  async function report(kind) {
    ...
    try {
      ...
      const msg = kind === "works"
        ? "Thanks! Marked as working."
        : kind === "approval"
        ? "Thanks — flagged as admin-approval. Will hide after another report."
        : "Thanks — flagged as invalid.";
      toast.success(msg);
      ...
      <div className="px-4 py-5 flex-1">
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-[11px] tracking-[0.15em] uppercase font-bold text-zinc-600">
          <span className="inline-flex items-center gap-1.5">
            ...
            {works > 0 ? `${works} confirmed` : "Unverified"}
          </span>
          <span className="inline-flex items-center gap-1.5">
            <Clock size={14} weight="bold" /> {timeAgo(group.last_checked)}
          </span>
          <span className="inline-flex items-center gap-1.5">
            {group.discovered_via === "user" ? "Submitted" : "Auto-found"}
          </span>
        </div>
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <button
            data-testid={`report-works-${group.id}`}
          ...
          <button
            data-testid={`report-approval-${group.id}`}
            onClick={() => report("approval")}
            disabled={!!reported}
            className="chip text-[10px] disabled:opacity-50"
          ...
          <button
            data-testid={`report-invalid-${group.id}`}
            onClick={() => report("invalid")}
            disabled={!!reported}
            className="chip text-[10px] disabled:opacity-50"
